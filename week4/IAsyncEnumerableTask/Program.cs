...
namespace GitHubActivityReport
{
    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                await foreach (var item in RunPagedQueryAsync(client, PagedIssueQuery, "docs", cancellationSource.Token, progressReporter))
                {
        ...
        private static async IAsyncEnumerable<JToken> RunPagedQueryAsync(GitHubClient client, string queryText, string repoName, CancellationToken cancel, IProgress<int> progress)
        {
            ...
            while (hasMorePages && (pagesReturned++ < 10))
            {
                ...
                // <SnippetProcessPage>
                yield return issues(results)["nodes"];
